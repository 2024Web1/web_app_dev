...
なお、データベース「php」のテーブル「person」を使用するので、**スクリプトファイル「   .txt」でpersonテーブルを再作成してください。** 
作り方を忘れた場合は、[09.データベースの作成]()を参照してください。

## クラス構成

データベースの基本事項に関するクラス `DbData` とそれを継承して実際にデータを操作するクラス `DbPhp` を定義する。
